...
// every deploy that forgot to set its own.
//
...
/**
 * Reads the configured UUIDs, keeping only well-formed ones. A typo silently
 * dropping a UUID is better than a malformed entry being matched loosely
 * against the value a client presents.
 * @param {string | undefined} rawUUID comma separated UUIDs from env.UUID
 * @returns {string[]} the valid UUIDs, or an empty array if none are usable
 */
export function parseUserIDs(rawUUID) {
	if (!rawUUID) return [];
	return rawUUID.split(',').map((id) => id.trim()).filter((id) => isValidUUID(id));
}
...
export default {
	...
	async fetch(request, env, ctx) {
		...
		try {
			const userIDs = parseUserIDs(env.UUID);
			if (userIDs.length === 0) {
				// Refusing outright beats falling back to anything: with no UUID
				// there is no credential, so every route below would be open.
				return new Response('ERR_UUID_UNCONFIGURED: set UUID to one or more valid UUIDs.', {
					status: 503,
					headers: { 'Content-Type': 'text/plain;charset=utf-8' },
				});
			}
			// Assigned outright rather than defaulted, so a request without a UUID
			// cannot inherit the one left behind by the request before it.
			userID = userIDs.join(',');
